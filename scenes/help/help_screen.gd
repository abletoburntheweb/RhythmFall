...
func _build_help_rows() -> Array:
	return [
		...
		{"section": SECTION_SERVER, "title": "Нужен ли локальный сервер для BPM и генерации?", "content": "Да. И [color=#%s]определение BPM[/color], и [color=#%s]генерация нот[/color] выполняются сервером (Python, Flask). Запускайте его из поставки [color=#%s]RhythmFallServer[/color]. В [color=#%s]Настройки → Прочее[/color]: режим [color=#%s]на этом компьютере[/color] ([color=#%s]localhost[/color]) или [color=#%s]другой ПК в сети[/color] с полем IP/имени хоста и портом (часто [color=#%s]5000[/color]). Без работающего процесса сервера анализ и генерация не завершатся." % [HC.teal, HC.primary, HC.muted, HC.sky, HC.teal, HC.teal, HC.sky, HC.teal]},
		...
		{"section": SECTION_GAMEPLAY, "title": "Как рассчитываются очки и комбо?", "content": "Очки начисляются за каждое попадание в зависимости от точности: [color=#%s]ИДЕАЛЬНО[/color] или [color=#%s]ХОРОШО[/color]. Комбо увеличивает ваш множитель очков каждые 10 успешных нажатий (до x4.0). Любое несвоевременное нажатие или пропуск ноты (ПРОМАХ) полностью сбрасывают серию комбо." % [HC.perfect, HC.good]},
