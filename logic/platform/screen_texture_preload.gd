...
static func warmup_active_cover_pack() -> void:
	...
	if loader == null:
		return
	var seen: Dictionary = {}
	for resource_path in TrackPlaceholderCover.warmup_paths():
		_request_path(loader, resource_path, seen)


static func warmup_shop_category(category: String, limit: int = 24) -> void:
	var loader: ThreadedTextureLoader = ThreadedTextureLoader.get_instance()
	if loader == null:
		return
	var seen: Dictionary = {}
	...
	for item in items:
		if not item is Dictionary:
			continue
		if category != "Все" and String(item.get("category", "")) != category:
			continue
		var ip := str(item.get("image", ""))
		_request_path(loader, ip, seen)
		if not ip.is_empty() and seen.has(ip):
