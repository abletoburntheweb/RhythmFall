# logic/utils/ui_category_button.gd
extends RefCounted
...
static func apply_selection(btn: Button, active: bool, icon_size: int = 14, compact: bool = false) -> void:
	...
	if configured:
		...
		_apply_configured_styleboxes(btn, active, accent, compact)
		_apply_font_colors(btn, active, accent)
		_refresh_icon(btn, active, icon_size, accent)
		...
		return
	var accent := _accent_for_button(btn)
	_apply_accent_border(btn, active, compact, accent)
	_apply_font_colors(btn, active, accent)
	_refresh_icon(btn, active, icon_size, accent)
	_UiMotionEffects.stop_control_border_pulse(btn)
	if active:
...
static func _apply_accent_border(btn: Button, active: bool, compact: bool, accent: Color) -> void:
	var states := ["normal", "hover", "pressed", "focus", "disabled"]
	if not active:
		var neutral := _neutral_inactive_stylebox(compact)
		for state in states:
			btn.add_theme_stylebox_override(state, neutral.duplicate())
		return
...
static func _apply_configured_styleboxes(btn: Button, active: bool, accent: Color, compact: bool) -> void:
	var box: StyleBoxFlat = make_accent_stylebox(accent, compact) if active else _neutral_inactive_stylebox(compact)
	var states := ["normal", "hover", "pressed", "focus", "disabled"]
	for state in states:
...
static func _apply_font_colors(btn: Button, active: bool, accent: Color) -> void:
...
static func _refresh_icon(btn: Button, active: bool, icon_size: int, accent: Color) -> void:
	if btn == null or not btn.has_meta(META_ICON):
		return
	var icon_file := str(btn.get_meta(META_ICON))
	if icon_file.strip_edges() == "":
		return
	var tint: Color = accent.lightened(0.12) if active else ICON_DIM
	UiIconHelper.configure_button_icon(btn, icon_file, tint, icon_size)
	btn.modulate = Color.WHITE if active else INACTIVE_MODULATE
