# logic/data/achievement_queue_manager.gd
extends Node
...
func _process_queue():
	...
	_show_achievement_notification(next_achievement, play_sound)
	if play_sound:
...
func _show_achievement_notification(achievement_data: Dictionary, play_sound := true):
	var root = get_tree().root
	var game_engine_node = root.get_node_or_null("GameEngine")
	var status_dock = null
	if game_engine_node:
		var notifications_layer = game_engine_node.get_node_or_null("NotificationsLayer")
		if notifications_layer:
			status_dock = notifications_layer.get_node_or_null("StatusDock")
	if status_dock and status_dock.has_method("show_achievement"):
		status_dock.show_achievement(achievement_data)
	else:
		print("AchievementQueueManager: StatusDock not found, falling back to legacy popup")
		_show_legacy_popup(achievement_data, play_sound)
		return
	if play_sound:
		_has_played_sound_in_cycle = true
	# StatusDock handles its own timing via clear timer
	get_tree().create_timer(5.5).timeout.connect(_on_notification_finished, CONNECT_ONE_SHOT)

func _show_legacy_popup(achievement_data: Dictionary, play_sound := true):
