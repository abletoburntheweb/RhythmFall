...
func _ready() -> void:
	...
	if ResourceLoader.exists(dbg_path):
		# Avoid double-loading if autoload already present
		if get_tree().root.get_node_or_null("DebugCommands") == null:
			var dbg = load(dbg_path).new()
			dbg.name = "DebugCommands"
			add_child(dbg)
			if dbg.has_method("_register"):
				dbg._register(self)
